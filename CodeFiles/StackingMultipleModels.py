...
#####Stacking with Lasso, XGB, RF, NN, Elastic

def stacking_Model(trainX,testX,trainY):
    #Split train data
    xFolds,yFolds,xSets,ySets=makeNSets(trainX,trainY,10)
    
    #Define models
    lassoModel=linear_model.Lasso(alpha=0.02, max_iter=6000)
    xgbModel = xgb.XGBRegressor(colsample_bytree=0.7,gamma=0.65,learning_rate=.2,max_depth=7,min_child_weight=1,
                 n_estimators=150,subsample=0.4)
    rfModel=RandomForestRegressor(n_estimators=40)
    nnModel=neural_network.MLPRegressor(hidden_layer_sizes=(50,50,50,50))
    elasticModel=ElasticNet(alpha=0.2,max_iter=15000)
    
    #Compute second level train data
    secLevelTrain=pd.DataFrame()
    secLevelTest=pd.DataFrame()
    for i in range(0, len(xSets)):
        fold=xSets[i]
        label=np.array(ySets[i])
        
        #lasso
        newMod=lassoModel.fit(fold,label)
        trainPre=pd.DataFrame(newMod.predict(trainX))
        testPre=pd.DataFrame(newMod.predict(testX))
        secLevelTrain=pd.concat([secLevelTrain,trainPre],axis=1)
        secLevelTest=pd.concat([secLevelTest,testPre],axis=1)

        #xgb
        newMod=xgbModel.fit(fold,label)
        trainPre=pd.DataFrame(newMod.predict(trainX))
        testPre=pd.DataFrame(newMod.predict(testX))
        secLevelTrain=pd.concat([secLevelTrain,trainPre],axis=1)
        secLevelTest=pd.concat([secLevelTest,testPre],axis=1)
        ...
        #elastic
        newMod=elasticModel.fit(fold,label)
        trainPre=pd.DataFrame(newMod.predict(trainX))
        testPre=pd.DataFrame(newMod.predict(testX))
        secLevelTrain=pd.concat([secLevelTrain,trainPre],axis=1)
        secLevelTest=pd.concat([secLevelTest,testPre],axis=1)
    ...
    secLevelLassoModel=linear_model.Lasso(alpha=0.02, max_iter=6000)
    secLevelLassoModel=secLevelLassoModel.fit(secLevelTrain,trainY)
    
    #Second level predictions
    testPrediction=pd.DataFrame(secLevelLassoModel.predict(secLevelTest))
...
testPrediction=stacking_Model(train_x,test_x,train_y)
...
testPrediction=testPrediction.rename(columns={0:'SalePrice'})
testPrediction.to_csv("Stacking.csv", index=False)
